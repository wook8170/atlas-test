...
import { calcJeonseBudget, calcSaleBudget, loanForPrice } from "../domain/loan";
...
export function BudgetPage() {
  ...
  const [dealType, setDealType] = useState<Exclude<DealType, "monthly">>("sale");
  ...
  const budget = useMemo(() => {
    if (!finance || !product) return null;
  ...
  const affordable = useMemo(() => {
    if (!finance || !product || !budget || !profile) return [];
    return listings
      ...
      .slice(0, 12);
  }, [listings, finance, product, budget, profile]);
